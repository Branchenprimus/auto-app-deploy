...
so keep it behind Cloudflare Access. App cards are discovered dynamically from
Ingress annotations generated by the shared Helm chart; use the optional
`homepage` block in an app manifest only for custom display metadata.
